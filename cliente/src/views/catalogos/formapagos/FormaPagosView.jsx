...
function FormaPagosView() {
  ...
  const obtenerFormasPagos = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_DEVICE_IP}/api/formaspago`
      );
      setFormasPagos(response.data);
    } catch (error) {
      console.error(error);
      toast.error("Error al obtener formas de pago");
    }
  };
  ...
  const filteredClientes = formasPagos.filter((item) =>
    item.nom_fmPago?.toLowerCase().includes(searchText.toLowerCase())
  );
    ...
    <>
      <section className="min-h-screen text-gray-800 bg-gray-50 dark:bg-darkMode-fondo dark:text-darkMode-font">
        <article className="flex items-center justify-between p-5">
          <h2 className="text-4xl font-semibold">Formas de pago</h2>
          ...
          <table className="w-full border-collapse bg-white shadow-lg rounded-lg text-xs">
            ...
            <tbody>
              {filteredClientes.map((item, i) => (
                ...
                >
                  <td className="px-6 py-3">{item.nom_fmPago}</td>
                  <td className="px-6 py-3">{item.desc_fmPago}</td>
                  <td className="px-6 py-3">
                    <Link
                      to={`/formas-pagos/edit/${item.id_fmPago}`}
