...
function TipoVentasView() {
  ...
  const obtenerTipoVentas = async () => {
    try {
      ...
      console.error(error);
      toast.error("Error al obtener tipo ventas");
    }
  };
  ...
  const filteredClientes = tipoVentas.filter((item) =>
            ...
            <tbody>
              {filteredClientes.map((item, i) => (
                ...
                >
                  ...
                  <td className="px-6 py-3">{item.desc_tpVenta}</td>
                  <td className="px-6 py-3">
                    <Link
                      to={`/tipoventas/edit/${item.id_tpVenta}`}
