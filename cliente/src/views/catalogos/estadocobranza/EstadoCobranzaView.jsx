...
function EstadoCobranzaView() {
  ...
  const filteredClientes = estados.filter((item) =>
    item.nom_estCobranza?.toLowerCase().includes(searchText.toLowerCase())
  );
    ...
    <>
      <section className="min-h-screen text-gray-800 bg-gray-50 dark:bg-darkMode-fondo dark:text-darkMode-font">
        <article className="flex items-center justify-between p-5">
          <h2 className="text-4xl font-semibold">Estados de cobranza</h2>
            ...
            <tbody>
              {filteredClientes.map((item, i) => (
                ...
                >
                  ...
                  <td className="px-6 py-3">{item.desc_estCobranza}</td>
                  <td className="px-6 py-3">
                    <Link
                      to={`/estados/edit/${item.id_acabado}`}
