...
function AcabadosView() {
  ...
  const obtenerAcabados = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_DEVICE_IP}/api/acabados`
      );
      setAcabados(response.data);
    } catch (error) {
      console.error(error);
      toast.error("Error al obtener acabados");
    }
  };
  ...
  const filteredClientes = acabados.filter((item) =>
    item.nom_acabado?.toLowerCase().includes(searchText.toLowerCase())
  );
          ...
          <table className="w-full border-collapse bg-white shadow-lg rounded-lg text-xs">
            ...
            <tbody>
              {filteredClientes.map((item, i) => (
                ...
                >
                  <td className="px-6 py-3">{item.nom_acabado}</td>
                  <td className="px-6 py-3">{item.desc_acabado}</td>
                  <td className="px-6 py-3">
                    <Link
                      to={`/acabados/edit/${item.id_acabado}`}
